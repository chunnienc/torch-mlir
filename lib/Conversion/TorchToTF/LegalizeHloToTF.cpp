/**
 * Copy from tensorflow/compiler/mlir/tensorflow/transforms/legalize_hlo.cc
 */
...
#include "stablehlo/stablehlo/dialect/BroadcastUtils.h"
#include "stablehlo/stablehlo/dialect/ChloOps.h"
...
PermutationAndShape
GetPermutationAndTransposedShape(llvm::ArrayRef<int64_t> permutation_array,
...
GetInversePermutationArray(llvm::ArrayRef<int64_t> permutation_array) {
...
GetInversePermutation(llvm::ArrayRef<int64_t> permutation_array,
                      ConversionPatternRewriter &rewriter) {
...
PermutationAndShape
GetInversePermutationAndShape(llvm::ArrayRef<int64_t> permutation_array,
                              ShapedType input_type,
                              ConversionPatternRewriter &rewriter) {
  SmallVector<int64_t, 4> inverse_permutation_array =
      GetInversePermutationArray(permutation_array);
...
template <int SupportedSpatialDims> struct ConvertNdConvOp {
  bool IsSupportedConvOp(mhlo::ConvolutionOp conv_op) const {
    ...
    if (num_spatial_dims != SupportedSpatialDims)
      return false;
...
public:
  ...
  LogicalResult
  matchAndRewrite(mhlo::ConvolutionOp conv_op, OpAdaptor adaptor,
    ...
    // Precision config
    if (!conv_op.getPrecisionConfig().has_value())
      return failure();
  ...
  matchAndRewrite(mhlo::ConvolutionOp conv_op, OpAdaptor adaptor,
    ...
    if (IsSupportedConvOp(conv_op, rewriter).failed()) {
      return rewriter.notifyMatchFailure(
          conv_op, "doesn't support to convert to ConvBackpropInputOp or "
                   "ResizeBilinearOp");
    }
...
public:
  ...
  LogicalResult
  matchAndRewrite(mhlo::SliceOp slice_op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
    auto begin =
...
public:
  ...
  LogicalResult
  matchAndRewrite(mhlo::DynamicSliceOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
...
template <typename Range> size_t Size(Range &&range) { return range.size(); }
...
public:
  ...
  LogicalResult
  matchAndRewrite(mhlo::DynamicUpdateSliceOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
...
bool MatchSingleIota(DenseIntElementsAttr dimensions, Value iota) {
  ...
  if (!iota_op || dimensions.getNumElements() != 1)
    return false;
...
bool MatchReshapedIota(DenseIntElementsAttr dimensions, Value iota) {
  if (dimensions.getNumElements() != 1)
    return false;
...
protected:
  ...
  NextTensorIndex(SmallVector<int64_t> index, ArrayRef<int64_t> shape,
                  int64_t fixed_axis) {
#ifndef NDEBUG
    assert(shape.size() == index.size());
...
template <typename T> class StridedArrayView; // Class requires specialization.
...
public:
  ...
  LogicalResult
  matchAndRewrite(mhlo::SortOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
    ...
    if (!keys_ty || !keys_ty.hasStaticShape() ||
      ...
      return rewriter.notifyMatchFailure(
          op, "only match for the case where the first operand has a static "
              "int/float shapeType");
...
LogicalResult MatchBinaryReduceFunction<void>(mlir::Region &function) {
  ...
  if (return_op.getNumOperands() != 1)
    return failure();
  if (return_op.getOperands().front() != body.getArgument(1))
    return failure();
  return success();
}
...
template <>
LogicalResult
rewriteNonMatchInitValue<TF::MaxOp, void>(mhlo::ReduceOp reduce_op, Value input,
                                          ConstOp reduction_indices,
                                          ConversionPatternRewriter &rewriter) {
  return failure();
}
...
template <>
LogicalResult
rewriteNonMatchInitValue<TF::MinOp, void>(mhlo::ReduceOp reduce_op, Value input,
                                          ConstOp reduction_indices,
                                          ConversionPatternRewriter &rewriter) {
  return failure();
}
...
public:
  ...
  LogicalResult
  matchAndRewrite(mhlo::ReduceOp reduce_op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
...
private:
  ...
  LogicalResult MatchReduceOpOperand(mhlo::ReduceOp reduce_op) const {
    ...
    if (!reduce_op.getInputs()[0].getType().isa<RankedTensorType>())
      return failure();
    if (!reduce_op.getType(0).isa<RankedTensorType>())
      return failure();
    return success();
  }
};
...
public:
  ...
  matchAndRewrite(mhlo::ReduceOp reduce_op, OpAdaptor adaptor,
    ...
    if (reduce_op.getInputs().size() != 2)
      return failure();
    if (reduce_op.getDimensions().getNumElements() != 1)
      return failure();
    ...
    if (!matchPattern(reduce_op.getInitValues().front(),
      ...
      return failure();
    if (!IsValueInitValue(operand_init))
      return failure();
...
public:
  ...
  LogicalResult
  matchAndRewrite(mhlo::IotaOp iota_op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
...
public:
  ...
  LogicalResult
  matchAndRewrite(mhlo::ReduceWindowOp rw, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
        ...
        [&rewriter,
         &operand_type](const std::optional<DenseIntElementsAttr> &attr) {
          ...
          if (attr->getElementType() != rewriter.getIntegerType(64))
            return false;
          if (attr->getSplatValue<APInt>().getSExtValue() != 1)
            return false;
          return true;
        };
    ...
    if (window_dimensions.size() != operand_type.getRank())
      return failure();
    ...
    for (int64_t i = 0, e = window_dimensions.size(); i < e; ++i) {
      ...
      if (window_dimension != operand_type.getShape()[i])
        return failure();
      cumulative_axis = i;
    }
    ...
    if (padding.size() != operand_type.getRank() * 2)
      return failure();
...
public:
  ...
  LogicalResult
  matchAndRewrite(mhlo::DivOp div_op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
    auto rw =
        dyn_cast_or_null<mhlo::ReduceWindowOp>(div_op.getLhs().getDefiningOp());
    if (!rw || rw->getNumResults() != 1)
      return failure();
...
LogicalResult CanonicalizeScatterUpdates(
  ...
  if (canonical_update_window_dims == update_window_dims)
    return success();
...
public:
  ...
  LogicalResult
  matchAndRewrite(mhlo::GatherOp gather_op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
  ...
  LogicalResult
  ConvertGatherOpToSlice(mhlo::GatherOp gather_op,
                         ConversionPatternRewriter &rewriter) const {
    Value operand = gather_op.getOperand();
    Value start_indices = gather_op.getStartIndices();
...
private:
  ...
  TransposeParams
  CanonicalizeOffset(ShapedType result_type,
                     ArrayRef<int64_t> original_offset_dims) const {
...
public:
  ...
  LogicalResult
  matchAndRewrite(mhlo::WhileOp while_op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
    // HLO WhileOp should have two regions: cond and body.
    if (while_op->getNumRegions() != 2)
      return failure();
...
public:
  ...
  LogicalResult
  matchAndRewrite(mhlo::ScatterOp scatter_op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
    ...
    if (operands.size() != 1 || updates.size() != 1)
      return failure();
...
public:
  ...
  LogicalResult
  matchAndRewrite(mhlo::PopulationCountOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const final {
